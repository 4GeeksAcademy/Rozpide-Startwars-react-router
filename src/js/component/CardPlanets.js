  ...
  return (
    <div className="d-flex overflow-auto" style={{ whiteSpace: "nowrap" }}>
      {store.planets.map((planet, index) => (
        <div className="card card-planets mx-2" key={index} style={{ minWidth: "200px" }}>
          <img
            src={`https://starwars-visualguide.com/assets/img/planets/${planet.url.split('/').slice(-2, -1)[0]}.jpg`}
            className="card-img-top"
            alt={planet.name}
          />
          <div className="card-body">
            ...
            <Link to={`/details/planets/${planet.url.split('/').slice(-2, -1)[0]}`} className="btn btn-primary mt-2">
              View Details
            </Link>
          </div>
        </div>
